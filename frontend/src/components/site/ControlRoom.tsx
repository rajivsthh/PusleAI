        ...
        <div className="flex items-end justify-between mb-10 gap-6 flex-wrap">
          <div>
            ...
            <h2 className="text-4xl sm:text-5xl font-display tracking-tight">
              ...
              for media intelligence.
            </h2>
          </div>
          <div className="text-xs text-muted-foreground font-mono">
            SYS · v2.6.1 · uptime 99.998%
          </div>
        </div>
              ...
              <div className="mt-3 grid grid-cols-5 gap-2 text-[10px] font-mono text-muted-foreground">
                {["fetch", "summarize", "storyboard", "render", "publish"].map((s, i) => (
                  <div key={s} className="flex items-center gap-1.5">
                    <span
                      className={`h-1.5 w-1.5 rounded-full ${i < 3 ? "bg-iris" : i === 3 ? "bg-iris/60 pulse-dot" : "bg-white/15"}`}
                    />
  ...
  return (
    <svg viewBox="0 0 100 80" className="w-full h-32">
      ...
      {nodes.slice(0, -1).map((n, i) => {
        const m = nodes[i + 1];
        return (
          <line key={i} x1={n.x} y1={n.y} x2={m.x} y2={m.y} stroke="url(#ln)" strokeWidth="0.4" />
        );
      })}
      {nodes.map((n, i) => (
        <g key={i}>
