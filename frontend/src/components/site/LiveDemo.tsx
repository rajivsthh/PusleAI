...
const stages = [
  {
    id: 0,
    label: "Headline ingested",
    icon: Sparkles,
    text: "NASA's Europa Clipper detects plumes near Jupiter's icy moon.",
  },
  {
    id: 1,
    label: "AI summarizing",
    icon: Wand2,
    text: "Drafting 38s narration · ne-NP · grade-9 reading level",
  },
  {
    id: 2,
    label: "Storyboard rendering",
    icon: Film,
    text: "7 scenes · captions · b-roll matched",
  },
      ...
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="mt-3 text-muted-foreground max-w-xl mx-auto">
            A real run, sped up. No edits, no humans in the loop.
          </p>
        </div>
            ...
            <div className="space-y-3">
              {stages.map((s, i) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`mt-0.5 flex h-7 w-7 items-center justify-center rounded-md border ${active ? "border-iris/50 bg-iris/15" : "border-white/10 bg-white/5"}`}
                    >
                      <Icon
                        className={`h-3.5 w-3.5 ${active ? "text-iris" : done ? "text-iris-soft" : "text-muted-foreground"}`}
                      />
                    </span>
            ...
            <div className="relative w-[260px] h-[520px] rounded-[36px] border border-white/10 bg-black overflow-hidden iris-glow">
              ...
              <div className="absolute inset-x-4 top-12 flex items-center justify-between text-[10px] font-mono text-white/70">
                ...
                <span className="flex items-center gap-1">
                  <span className="h-1.5 w-1.5 rounded-full bg-iris pulse-dot" /> live
                </span>
              </div>
              <div className="absolute inset-x-4 bottom-20 text-white">
                <div className="text-[11px] font-mono uppercase tracking-[0.18em] text-white/60">
                  Scene {Math.min(step + 1, stages.length)}/4
                </div>
              ...
              <div className="absolute right-3 bottom-24 flex flex-col items-center gap-3 text-white/80 text-[10px]">
                {["♥", "↗", "✚"].map((g, i) => (
                  <div
                    key={i}
                    className="h-9 w-9 rounded-full glass-strong grid place-items-center text-base"
                  >
                    {g}
                  </div>
                ))}
