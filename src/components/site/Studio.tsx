      ...
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="mt-3 text-muted-foreground max-w-xl mx-auto">
            A calm dashboard for the chaos of breaking tech news.
          </p>
        </div>
            ...
            <div className="grid md:grid-cols-[1fr_280px]">
              ...
              <div className="divide-y divide-white/5">
                {queue.map((q) => (
                  <div
                    key={q.title}
                    className="px-5 py-3.5 flex items-center gap-4 hover:bg-white/[0.02] transition"
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-3 mt-0.5 text-[11px] text-muted-foreground font-mono">
                        <span className="flex items-center gap-1">
                          <CheckCircle2 className="h-3 w-3 text-iris" /> {q.score}% confidence
                        </span>
                        <span className="flex items-center gap-1">
                    ...
                    <span
                      className={`text-[10px] font-mono uppercase tracking-[0.15em] px-2 py-1 rounded-full border ${
                        q.status === "ready"
                          ? "border-iris/30 text-iris bg-iris/5"
                          : q.status === "review"
                            ? "border-amber-300/30 text-amber-200 bg-amber-300/5"
                ...
                <div>
                  ...
                  <div className="flex items-end gap-1.5 h-16">
                    {[24, 38, 52, 30, 64, 42, 70, 58, 80, 66, 90, 74].map((h, i) => (
                      <div
                        key={i}
                        className="flex-1 rounded-sm bg-gradient-to-t from-iris/40 to-iris/80"
                        style={{ height: `${h}%` }}
                      />
                    ))}
                  </div>
                    ...
                    {Array.from({ length: 48 }).map((_, i) => {
                      ...
                      return (
                        <div
                          ...
                          style={{ background: `oklch(0.72 0.16 285 / ${0.08 + v * 0.55})` }}
                        />
                      );
                    })}
                  </div>
